...
```{r}
# 设置CRAN镜像为清华大学镜像源（提升R包下载速度）
# Set CRAN mirror to Tsinghua University mirror (improve R package download speed)
options("repos"= c(CRAN="https://mirrors.tuna.tsinghua.edu.cn/CRAN/"))

# 设置Bioconductor镜像为清华大学镜像源（用于生物信息学相关R包）
# Set Bioconductor mirror to Tsinghua University mirror (for bioinformatics-related R packages)
...
The tcga_sthy450 folder contains methylation data, filtered out probes with null values, and saved them as RData files. Weiyun link:< https://share.weiyun.com/jRry4rlU >After downloading, place the tcga_cethy450 folder into the current folder. The same file is used as FigureYa270panMet.
...
`The TCGA_cncan_correlation-expr_eth_stubset. txt file contains n * n correlations and p-values, which can be applied to the FigureYa we previously crowdfunded. More ways to display relevance can be found here: [Which of the 8 situations to display relevance would you like?]（ https://mp.weixin.qq.com/s/D9wheY5QdnOh4JrjIc8-Cg )
