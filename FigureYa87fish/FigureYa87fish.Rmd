...
fishPlot(fish, shape = "spline",         # 曲线类型：spline(平滑曲线)或polygon(折线)
         #title.btm = "Clonal architecture of tumors",  # 左下角标题(已注释)
         cex.title = 1.2,               # 标题字号大小
         pad.left = 0.25,               # 鱼头部分的倾斜角度参数
         
         vlines = timepoints[c(1:2,4)],  # 在指定时间点绘制垂直参考线
         col.vline = "white",           # 参考线颜色
         vlab = c("PMF", "sAML", "sAML\nREM"),  # 垂直参考线对应的标签
         
         bg.col = c("#F1F2F2","#F1F2F2","#F1F2F2"),  # 背景颜色设置
         border = 0.1                   # 克隆轮廓线宽度
         )
...
par(xpd = T)  # 允许图形元素绘制在绘图区域外
legend("bottomright", 
       inset=c(.7,-.3),  # 控制图例位置(调整以适应图形)
       pch=16, bty="n",  # 点形状和无边框
       col=fish@col, text.col = fish@col,  # 使用克隆对应颜色
       legend = paste0(row.names(frac)," ",frac$PMF,"%"))  # 图例内容
