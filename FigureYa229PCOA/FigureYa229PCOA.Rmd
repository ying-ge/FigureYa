...
```{r}
# 计算距离矩阵（非相似性矩阵）
brca.dist <- vegdist(t(expr), # 注意行为观测（样本），列为特征（基因）
                     method = "euclidean") # 原文使用Bray-Curtis dissimilarity matrix针对非负矩阵(method = "bray")，这里表达谱有负值，所以采用普通的欧氏距离
...
xlab.text = paste("PCoA1 [", pct1, "%]", sep = "") # 生成x轴标签
ylab.text = paste("PCoA2 [", pct2, "%]", sep = "") # 生成y轴标签
```
