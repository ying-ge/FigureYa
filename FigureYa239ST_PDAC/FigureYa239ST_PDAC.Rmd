...
作者：@ 西红柿，他的更多作品看这里<https://k.koudai.com/Bv3Q7WYj>
...
```{r}
# 首先是PDAC-A空间转录组数据的读入
dataA <- fread("GSE111672_RAW/GSM3036911_PDAC-A-ST1-filtered.txt/GSM3036911_PDAC-A-ST1-filtered.txt", header = T,check.names = F)
...
结论：After clustering the spots of each ST array based on the principal component scores (see Methods), we found that the resulting clusters were consistent with the independent histological annotations (Fig. 2e,f and Supplementary Fig. 5g,h), 
...
```{r}
## 读取PDAC-B空间转录组数据
dataA <- fread("GSE111672_RAW/GSM3405534_PDAC-B-ST1-filtered.txt/GSM3405534_PDAC-B-ST1-filtered.txt", header = T,check.names = F)
dataA <- as.data.frame(dataA)
...
```{r}
scdataA <- fread("GSE111672_PDAC-B-indrop-filtered-expMat.txt/GSE111672_PDAC-B-indrop-filtered-expMat.txt",header=T)
...
scRNA_marker=FindAllMarkers(scRNA,test.use = "t",
                            only.pos = T,
                            logfc.threshold = 0.05)
#原文根据p值提取marker基因集，我尝试了原文的阈值实在是太少了，所以这里就保留
scRNA_marker2=subset(scRNA_marker)
table(scRNA_marker2$cluster)
